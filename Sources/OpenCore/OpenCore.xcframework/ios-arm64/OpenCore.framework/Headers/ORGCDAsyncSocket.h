...
@class ORGCDAsyncReadPacket;
@class ORGCDAsyncWritePacket;
@class ORGCDAsyncSocketPreBuffer;
@protocol ORGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const ORGCDAsyncSocketException;
extern NSString *const ORGCDAsyncSocketErrorDomain;

extern NSString *const ORGCDAsyncSocketQueueName;
extern NSString *const ORGCDAsyncSocketThreadName;

extern NSString *const ORGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const ORGCDAsyncSocketUseCFStreamForTLS;
#endif
#define ORGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define ORGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define ORGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const ORGCDAsyncSocketSSLPeerID;
extern NSString *const ORGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const ORGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const ORGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const ORGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const ORGCDAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const ORGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define ORGCDAsyncSocketLoggingContext 65535
...
@interface ORGCDAsyncSocket : NSObject

- (instancetype)init;
- (instancetype)initWithSocketQueue:(nullable dispatch_queue_t)sq;
- (instancetype)initWithDelegate:(nullable id<ORGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<ORGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create ORGCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<ORGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<ORGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<ORGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<ORGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<ORGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<ORGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<ORGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means ORGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means ORGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, ORGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, ORGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of ORGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a ORGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a ORGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that ORGCDAsyncSocket does NOT copy the data. It simply retains it.
...
@protocol ORGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(ORGCDAsyncSocket *)sock;
...
**/
- (void)socket:(ORGCDAsyncSocket *)sock didAcceptNewSocket:(ORGCDAsyncSocket *)newSocket;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)socket:(ORGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(ORGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(ORGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(ORGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(ORGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(ORGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(ORGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(ORGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(ORGCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the ORGCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(ORGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socket:(ORGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
